...
unset OPENCODE_WEB_RETENTION_DAYS OPENCODE_WEB_RETENTION_POLL_SECONDS OPENCODE_WEB_RETENTION_FETCH_TIMEOUT_MS OPENCODE_WEB_RETENTION_VERIFY_TIMEOUT_MS || true
mkdir -p "${HOME}"
...
assert_contains "$output" "retention_days=0"
assert_contains "$output" "retention_dry_run=0"
...
assert_contains "$output" "retention_schedule=after-health-at-most-weekly"
assert_contains "$output" "session-retention.last-success"
assert_contains "$output" "image_playwright_version=disabled"
assert_contains "$output" "image_playwright_expected_version=1.62.1"
...
assert_contains "$output" "workspace_ui_state_scope=browser-local-storage"
